...
function initCalendar(customMonth, customYear) {
  month = customMonth !== undefined ? customMonth : month;
  year = customYear !== undefined ? customYear : year;
...
function gotoDate() {
  ...
  if (dateArr.length === 3) {
    ...
    const monthInput = parseInt(dateArr[1]) - 1; // Adjust index for month
    const yearInput = parseInt(dateArr[2]);
    if (day >= 1 && day <= 31 && monthInput >= 0 && monthInput <= 11 && yearInput >= 1000 && yearInput <= 9999) {
      initCalendar(monthInput, yearInput);
      // After initializing the calendar, set the active day
      setActiveDay(day);
      return;
    }
  }
...
function setActiveDay(day) {
  ...
  days.forEach(dayElem => {
    if (parseInt(dayElem.textContent) === day) {
